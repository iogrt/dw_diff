CREATE TABLE t_dim_customer(
   customer_key			   NUMBER(9),
   customer_natural_key	   NUMBER(8),
   customer_card_number 	VARCHAR2(20),
   customer_name			   VARCHAR2(40),
   customer_address		   VARCHAR2(60),
   customer_location		   VARCHAR2(60),
   customer_district		   VARCHAR2(40),
   customer_zip_code		   VARCHAR2(8),
   customer_phone_nr		   NUMBER(9),
   customer_gender		   VARCHAR2(15),
   customer_gender_old     VARCHAR2(15),
   customer_age			   NUMBER(3),	
   customer_marital_status	VARCHAR2(15),
   is_expired_version	   VARCHAR2(3),	-- {'NO'=current version; 'YES'=expired version}
   CONSTRAINT pk_tdimcustomer_key PRIMARY KEY (customer_key)
);
...
CREATE TABLE t_dim_date(
	...
	date_year		NUMBER(4),
	date_temperature_status VARCHAR2(20),
...
CREATE TABLE t_fact_lineofsale(
	...
	CONSTRAINT fk_tFactlineofsale_customerkey FOREIGN KEY (customer_key) REFERENCES t_dim_customer(customer_key)
);
